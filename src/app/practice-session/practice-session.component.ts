...
export class PracticeSessionComponent implements OnInit {
  public readonly words: InputSignal<WordDTO[]> = input.required();
...
this.nextWord();
  ...
  public correct(){
    if(this.currentWord.score < 5){
      ...
      this.separatedWords[this.currentWordArrayIndex + 1].push(this.currentWord)
      this.separatedWords[this.currentWordArrayIndex].splice(this.currentWordIndex, 1);
    }
    this.nextWord();
  }
  ...
  private setNewCurrentWord(){
    ...
    else{this.previousWords.push(this.currentWord.id)}
  ...
  private generateSemiRandomWord(){
    ...
    for(let i = this.spread; i > 0; i--){
      ...
      const numsInArr = this.separatedWords[this.currentWordArrayIndex].length * i;
      if(generatedNumber < numsInArr ){
        this.currentWordIndex = generatedNumber % this.separatedWords[this.currentWordArrayIndex].length;
       return (this.separatedWords[this.currentWordArrayIndex][this.currentWordIndex]);
      }
